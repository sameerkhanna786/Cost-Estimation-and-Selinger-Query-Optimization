...
public class Histogram {
  ...
  public void buildHistogram(Table table, int attribute){
    ...
    while (iter.hasNext()) {
      ...
      float score = quantization(r, attribute);
      if (score > max) {
        max = score;
      }
      if (score < min) {
    ...
    for (int i = 0; i < numBuckets; i++) {
      float end = current + width;
      buckets[i] = new Bucket<Float>(current, end);
      current = end;
    }
    ...
    while (iter.hasNext()) {
      Record r = (Record) iter.next();
      float quant = quantization(r, attribute);
      buckets[bucketIndex(quant)].increment(quant);
    }
  }
  ...
  private float [] allEquality(float qvalue){
    ...
    result[index] = ((float) 1)/((float) buckets[index].getDistinctCount());
    return result;
  }
 ...
 /**
  ...
  private float [] allNotEquality(float qvalue){
    ...
    int index = bucketIndex(qvalue);
    result[index] = ((float) 1) - ((float) 1)/((float) buckets[index].getDistinctCount());
    return result;
  }
  ...
  private float [] allGreaterThan(float qvalue){
    ...
    int index = bucketIndex(qvalue);
    result[index] = ((buckets[index].getEnd() - qvalue)/(width));
 ...
 /**
  ...
  private float [] allLessThan(float qvalue){
    ...
    int index = bucketIndex(qvalue);
    result[index] = (qvalue - buckets[index].getStart())/(width);
    for (int i = 0; i < index; i++) {
      result[i] = 1;
    }
    for (int i = index+1; i < numBuckets; i++) {
